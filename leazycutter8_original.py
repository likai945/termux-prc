...
def chuli_vmjzfz(chu, zhong, dxwz):
    if os.path.exists(chu):
        with open(zhong, "a", encoding='utf_8_sig') as wen:
            dct = bianli(chu, dxwz)
            for vnf in dct:
                for vm in dct[vnf]:
                    vmavg = sum(dct[vnf][vm]) / len(dct[vnf][vm])
                    vmmax = max(dct[vnf][vm])
                    vmavg = str(vmavg)
                    vmmax = str(vmmax)
                    jieguo = f'{vnf},{vm},{vmavg},{vmmax}\n'
                    wen.write(jieguo)
...
chuli_vmjzfz('sbmkx3.csv', 'vmjzfz3.csv', 3)
print("AvgVM and MaxVM of kx3 done.")
chuli_vmjzfz('sbmkx4.csv', 'vmjzfz4.csv', 3)
print("AvgVM and MaxVM of kx4 done.")
chuli_vmjzfz('sbmkx5.csv', 'vmjzfz5.csv', 3)
