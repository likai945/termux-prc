...
def prd_his_alllines(his3, his4, his5, his9, his11):
    ...
    lines11 = one_line_to_write(dct11)
    alllines = [lines3[0], lines4[0], lines5[0], lines9[0], lines11[0]]

    global htotal3, htotal4, htotal5, htotal9, htotal11
...
def prd_crt_alllines(crt3, crt4, crt5, crt9, crt11):
    ...
    lines9 = one_line_to_write(dct9,clrt9)
    lines11 = one_line_to_write(dct11,clrt11)
    alllines = [lines3[0], lines4[0], lines5[0], lines9[0], lines11[0]]

    global ctotal3, ctotal4, ctotal5, ctotal9, ctotal11
...
def write_contents(sheet, hisorcrt):
    if hisorcrt == 'his':
        alllines = prd_his_alllines(his3, his4, his5, his9, his11)
    elif hisorcrt == 'crt':
        alllines = prd_crt_alllines(crt3, crt4, crt5, crt9, crt11)
    r = 2
    rs = []
...
def write_lframe(sheet, rows, title):
    ...
    if '11' in nelst:
        sheet.write(rows[3], 1, '可信11', cfmt)
    else:
        sheet.merge_range(rows[3], 1, rows[4] - 1, 1, '可信11', cfmt)
...
def write_smr_sheet(sheet):
    ...
    sheet.merge_range(1, 0, 5, 0, date, cfmt)
    ...
    linekx11 = ['可信11', htotal11, ctotal11, ctotal11, 0]
    ...
    sheet.write_row(5, 1, linekx11, cfmt)
...
def check_files_exist():
    for i in files:
        if not os.path.exists(i):
            print(f'\n\n文件 {i} 不存在或命名错误，请检查。')
            print(f'\n若资源池{i[1:-4]}没有{lang[i[0]]}告警，请输入 "OK" 以确定。')
            print(f'若命名错误，请按下 <回车键> 以退出，重命名为{i}后再次执行。')
            opt = input('请确认：')
            if opt == 'ok' or opt == 'OK':
                continue
            else:
...
def collect_clear_times():
    global clrt3,clrt4,clrt5,clrt9,clrt11
    clrt3 = clrt4 = clrt5 = clrt9 = clrt11 = 'na'
    ...
    if os.path.exists(crt9):
        clrt9=input("DMZ9当前告警清除时间：")
    if os.path.exists(crt11):
        clrt11=input("可信11当前告警清除时间：")
...
his3, his4, his5, his9, his11 = 'h3.csv','h4.csv','h5.csv','h9.csv','h11.csv'
crt3, crt4, crt5, crt9, crt11 = 'c3.csv', 'c4.csv', 'c5.csv', 'c9.csv', 'c11.csv'
files = [his3, his4, his5, his9, his11, crt3, crt4, crt5, crt9, crt11]
