...
my(){
cp $jl $tz $HOME/termux-app && cd $HOME/termux-app && git add . && git commit -m "daily_sync_`date +%Y%b%e%X`" &> /dev/null && git push &> /dev/null && return 0
}
dxa(){
...
gh(){
cp $jl $tz $HOME/termux-prc && cd $HOME/termux-prc && git add . && git commit -m "daily_sync_`date +%Y%b%e%X`" &> /dev/null && git push &> /dev/null && return 0
}
dxc(){
...
do
	read -p 'weight:' w
	[ $w -ge 100 ] &> /dev/null && [ $w -le 150 ] &> /dev/null && echo "${zheci} $w" >> ${tz} && rl=10 && break
