...
// Feb 4, 2026 change the way to get indexes
// Feb 10, 2026 change the logic of check files
// Feb 11, 2026 change the config file
// by LiKai
...
func getFile(pool string) [3]string {
	re := regexp.MustCompile("[0-9]+")
	num := re.FindString(pool)
	hfile := fmt.Sprintf("h%s.csv", num)
	cfile := fmt.Sprintf("c%s.csv", num)
	files := [3]string{hfile, cfile, num}
	return files
}
...
func do() {
	for _, pool := range pools {
		...
		num := getFile(pool)[2]
		writeSheet("Sheet2", pool, hisr, build(hfile, pool))
		writeSheet("Sheet3", pool, crtr, build(cfile, pool))
		writeSheet("Sheet1", pool, smrr, getKey(num))
	}
}
...
func unifyPc(arr []string) []string {
	for _, one := range arr {
		if one != arr[0] {
			return arr
		}
...
func getConf() {
	predict := toDict("config")
	for key, val := range predict {
		config[key] = val
	}
	pools = strings.Split(config["pools"], ",")
}
...
func build(file, pool string) [][]string {
...
func checkExist(file, pool string) {
	hcchmap := map[string]string{"h": "历史", "c": "当前"}
	fexist, _ := filepath.Glob(file)
	var command string
	if len(fexist) == 0 && checkEnn(file) {
		fmt.Printf("未发现\033[42m%s\033[0m资源池\033[32m%s\033[0m告警，若该资源池确无\033[32m%s\033[0m告警，请请输入OK以确认，否则输入NG以退出。\n", pool, hcchmap[file[0:1]], hcchmap[file[0:1]])
		fmt.Scanln(&command)
...
func checkEnn(file string) bool {
	for _, chdname := range namemap {
		if file == chdname {
			return false
		}
	}
...
func checkExists() {
	for _, pool := range pools {
		hfile := getFile(pool)[0]
		checkExist(hfile, pool)
		cfile := getFile(pool)[1]
		checkExist(cfile, pool)
	}
}

func toDict(file string) map[string]string {
	var resarr [][]string
	amap := map[string]string{}
	if fileExist(file) {
		resarr = readFile(file)
	} else {
		return amap
	}
...
func mvFile(file, dirname string) {
...
func unzipZiles() {
	...
	for _, zipfile := range files {
		...
		tmpfiles = append(tmpfiles, csvname)
...
func renameZiles() {
	for oldname, newname := range namemap {
		zoldname := strings.Replace(oldname, "csv", "zip", -1)
		znewname := strings.Replace(newname, "csv", "zip", -1)
		os.Rename(zoldname, znewname)
	}
}

func getPn(file string) string {
	cont := readFile(file)
...
func getPns() {
	...
	for _, csvfile := range files {
		poolnum := toNum(getPn(csvfile))
		prefix := csvfile[0:1]
		newcname := prefix + strconv.Itoa(poolnum) + ".csv"
		if poolnum != 0 {
			namemap[csvfile] = newcname
		}
	}
	remind()
}
